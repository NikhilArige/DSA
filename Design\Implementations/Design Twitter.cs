...
public class Twitter {
    ...
    public IList<int> GetNewsFeed(int userId) {
        ...
        var followees = new HashSet<int>();
            if (map.ContainsKey(userId)){
                followees = map[userId];
            }
                 
        int cnt = 0;
        for(int i=n-1;i>=0 && cnt<10;i--){
            if (tweets[i].userId == userId || followees.Contains(tweets[i].userId))
                {
                    res.Add(tweets[i].tweetId);
                    cnt++;
                }
        }
        return res;        
    ...
    public void Unfollow(int followerId, int followeeId) {
         if (map.ContainsKey(followerId) &&  map[followerId].Contains(followeeId))
            {
