...
where releaseTimes[i] was the time the ith key was released. Both arrays are 0-indexed. The 0th key was pressed at the time 0, and every subsequent key was pressed at the exact
...
public class Solution {
    public char SlowestKey(int[] releaseTimes, string keysPressed) {
        ...
        var curmin = releaseTimes[0];
            for(int i=1;i<releaseTimes.Length;i++){
                if(curmin < releaseTimes[i]-releaseTimes[i-1]){
                    curmin = releaseTimes[i]-releaseTimes[i-1];
                    res = keysPressed[i]; 
                }
                if(curmin == releaseTimes[i]-releaseTimes[i-1]){
                    res = (res-'a')>(keysPressed[i]-'a') ? res : keysPressed[i];
                }
            }
        return res;
    }
