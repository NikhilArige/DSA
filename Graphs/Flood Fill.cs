...
public class Solution {
    public int[][] FloodFill(int[][] grid, int sr, int sc, int newColor) {
        ...
        dfs(sr,sc,newColor,grid[sr][sc],grid,check);
        return grid;
    }
    
    private void dfs(int i,int j,int nc,int cc,int[][] grid,bool[,] check){
        if(i<0 || j<0 || i>=grid.Length || j>=grid[0].Length ||
          grid[i][j]!=cc || check[i,j]){
            return;
        }
        grid[i][j] = nc;
        check[i,j] = true;
        dfs(i,j+1,nc,cc,grid,check);
        dfs(i,j-1,nc,cc,grid,check);
        dfs(i+1,j,nc,cc,grid,check);
        dfs(i-1,j,nc,cc,grid,check);
        return;
    }
}
