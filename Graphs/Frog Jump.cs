...
public class Solution {
    public bool CanCross(int[] stones) {
        ...
        var dest = stones[n-1];
        var queue = new Queue<(int pos,int steps)>();
        var hash = new HashSet<(int pos,int steps)>();
        queue.Enqueue((0,0));
        while(queue.Any())
        {
            var cur = queue.Dequeue();
            for(int i=cur.steps-1;i<=cur.steps+1;i++)
            {
                if(i<=0){
                    continue;
                } 
                var nextpos = cur.pos + i;
                if(nextpos == dest)
                {
                    return true;
                }
                if(!hash.Contains((nextpos,i))){
                    hash.Add((nextpos,i));
                }
                else{
                    continue;
